...
def materialize_properties_task(
    ...
    properties_to_materialize_by_table: dict[TableWithProperties, set[tuple[TableColumn, PropertyName]]] = defaultdict(
        set
    )
    for table, table_column, property_name in properties_to_materialize:
        properties_to_materialize_by_table[table].add((table_column, property_name))

    result: list[Suggestion] = []
    for table, properties_to_materialize_in_table in properties_to_materialize_by_table.items():
        properties_already_materialized = {
            (column.details.table_column, column.details.property_name) for column in MaterializedColumn.get_all(table)
        }
        for table_column, property_name in properties_to_materialize_in_table - properties_already_materialized:
